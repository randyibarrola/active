                        ...
                        <div class="panel panel-primary">
                            ...
                            <div class="panel-body">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-6"><p>{$noches} noche{if $noches > 1}s{/if}</p></div>
                                    </div>
                                    <div class="row">
                                    ...
                                    <div class="row">
                                        <div class="col-md-6"><p><strong>Email del cliente</strong></p></div>
                            ...
                            <p><strong>{if $hotel->tipoReserva eq 'online' && $hotel->porcentageCobroAnticipado > 0}✓ {#garantia_de_tu_reserva#}{else} {/if} {$apartamento->nombre} {if $hotel->tipoReserva eq 'online' && $hotel->porcentageCobroAnticipado > 0}{$hotel->porcentageCobroAnticipado}%{/if} <span id="cobraremos-tarjeta">{$reserva->moneda->simbolo}{if $hotel->tipoReserva eq 'online' && $hotel->porcentageCobroAnticipado > 0}{($apartamento->precioUnitario*$hotel->porcentageCobroAnticipado/100)|number_format:2:',':''}{else}{$apartamento->precioUnitario|number_format:2:',':''}({#Pagarias_en_tu_llegada_al_hotel#}){/if}</span></strong></p>
                                    ...
                                    {if $reserva->extras}
                                        {foreach from=$reserva->extras item=exc}
                                            ...
                                            <p id="cobrar_{$exc->id}">
                                            {if $exc->pagoOffline}
                                                {$exc->nombre} <span>{$reserva->moneda->simbolo}{$exc->precioUnitario|number_format:2:',':' '}(lo pagarías {$exc->modoPagoOffline})</span>
                                            {else}
                                                ✓ {$exc->nombre} {$exc->porcientoInicial}% <span>{$reserva->moneda->simbolo}{$exc->importeInicial|number_format:2:',':''} {if $exc->porcientoInicial < 100}(pagarías el resto {$exc->modoPagoOffline}){/if}</span>
                                            {/if}
                                            </p>
                                        {/foreach}
                                    {/if}
<hr />
                ...
                <br />
                 <div class="row" style="text-align: center;">
                    ...
                    <a href="{$base_url}/index{$end_url}" class="btn btn-success">{#finalizar#}</a>
                </div>
            </div>
        </div>
    </div>
